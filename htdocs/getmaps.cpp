...
int main()
{
	...
	dprintf("<tr><th align=\"left\">Name</th><th align=\"left\">Games</th><th align=\"left\">Draws (%%)</th></tr>\n");
	unsigned tg = 0, td = 0;
	unsigned i = 0;
	for (map<string, pair<unsigned, unsigned> >::iterator m = maps.begin();
		...
		tg += g;
		td += d;
		dprintf("<tr bgcolor=\"%s\"><td><a href=\"%s\">%s</a></td><td align=\"right\">%u</td><td align=\"right\">%u (%u%%)</td></tr>\n", bg, m->first.c_str(), m->first.c_str(), g, d, g > 0 ? d * 100 / g : 0);
	}
	const char *bg = i++ % 2 ? "#FFFFFF" : "#DDDDFF";
